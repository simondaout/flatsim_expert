...
"""
invers_disp2coef.py — Spatial and temporal inversions of InSAR cumulative time series.
...
parser = argparse.ArgumentParser(
    description="Spatial and temporal inversions of cumulative InSAR time series.",
    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
)
parser.add_argument("--cube", default=None, help="Path to time series cube (BIP format or GeoTIFF)")
parser.add_argument("--lectfile", default="lect.in", help="Path to lect.in (width, length, nimg)")
parser.add_argument("--list_images", default="images_retenues", help="Path to list_images file")
parser.add_argument("--aps", default=None, help="Path to APS file (date, aps per line). Pass 'none' to disable.")
parser.add_argument("--rms", default=None, help="Path to RMS file (date, rms per line). Pass 'none' to disable.")
parser.add_argument("--rmspixel", default=None, help="Path to RMS pixel map (r4 or tiff)")
parser.add_argument("--threshold_rms", default=1., type=float, help="Threshold on rmspixel for spatial estimations")
parser.add_argument("--ref_zone", default=None, help="Reference zone: jstart,jend,istart,iend")
parser.add_argument("--niter", default=0, type=int, help="Number of iterations")
parser.add_argument("--spatialiter", default="yes", help="Iterate spatial estimations at each iter (yes/no)")
parser.add_argument("--linear", default="yes", help="Add linear term (yes/no)")
parser.add_argument("--steps", default=None, help="Heaviside step times e.g. 2004.,2006.")
parser.add_argument("--postseismic", default=None, help="Log characteristic times, same length as steps e.g. 1.,1.")
parser.add_argument("--seasonal", default="no", help="Add seasonal terms (yes/no)")
parser.add_argument("--semianual", default="no", help="Add semi-annual terms (yes/no)")
parser.add_argument("--seasonal_increase", default=None, help="Add seasonal terms function of time")
parser.add_argument("--bianual", default="no", help="Add bi-annual terms (yes/no)")
parser.add_argument("--slowslip", default=None, help="Slow-slip: median and char. time e.g. 2004.,1,2006,0.5")
parser.add_argument("--vector", default=None, help="Path to vector text file (one value per date)")
parser.add_argument("--bperp", default="no", help="Add perpendicular baseline term (yes/no)")
parser.add_argument("--ivar", default=0, type=int, help="Phase/elevation relationship: 0=elevation, 1=az x elev")
parser.add_argument("--nfit", default=0, type=int, help="Fit degree: 0=linear, 1=quadratic")
parser.add_argument("--flat", default=0, type=int, help="Spatial ramp order (0-9)")
parser.add_argument("--sampling", default=1, type=int, help="Downsampling factor for temporal decomposition")
parser.add_argument("--emp_sampling", default=1, type=int, help="Downsampling factor for empirical estimations")
parser.add_argument("--imref", default=1, type=int, help="Reference image number (1-based)")
parser.add_argument("--cond", default=1e-6, type=float, help="SVD condition number")
parser.add_argument("--ineq", default="yes", help="Sequential least-square optimisation (yes/no)")
parser.add_argument("--mask", default=None, help="Path to mask file (r4 or tif)")
parser.add_argument("--rampmask", default="no", help="Remove quadratic ramp on mask (yes/no)")
parser.add_argument("--threshold_mask", default=1., type=float, help="Threshold on mask")
parser.add_argument("--scale_mask", default=1., type=float, help="Scale factor for mask")
parser.add_argument("--tempmask", default="no", help="Use mask for temporal decomposition (yes/no)")
parser.add_argument("--topofile", default=None, help="Path to topographic file (r4 or tif)")
parser.add_argument("--aspect", default=None, help="Path to aspect file (r4 or tif)")
parser.add_argument("--perc_los", default=99., type=float, help="Percentile of LOS pixels for spatial estimations")
parser.add_argument("--perc_topo", default=99., type=float, help="Percentile of topography for spatial estimations")
parser.add_argument("--crop", default=None, help="ROI for temporal decomposition: l0,l1,c0,c1")
parser.add_argument("--crop_emp", default=None, help="ROI for spatial estimation: l0,l1,c0,c1")
parser.add_argument("--fulloutput", default="no", help="Produce full output maps (yes/no)")
...
arguments = {
    "--cube": args.cube,
    "--lectfile": args.lectfile,
    "--list_images": args.list_images,
    "--aps": args.aps,
    "--rms": args.rms,
    "--rmspixel": args.rmspixel,
    "--threshold_rms": args.threshold_rms,
    "--ref_zone": args.ref_zone,
    "--niter": args.niter,
    "--spatialiter": args.spatialiter,
    "--linear": args.linear,
    "--steps": args.steps,
    "--postseismic": args.postseismic,
    "--seasonal": args.seasonal,
    "--semianual": args.semianual,
    "--seasonal_increase": args.seasonal_increase,
    "--bianual": args.bianual,
    "--slowslip": args.slowslip,
    "--vector": args.vector,
    "--bperp": args.bperp,
    "--ivar": args.ivar,
    "--nfit": args.nfit,
    ...
    "--emp_sampling": args.emp_sampling,
    "--imref": args.imref,
    "--cond": args.cond,
    "--ineq": args.ineq,
    "--mask": args.mask,
    "--rampmask": args.rampmask,
    "--threshold_mask": args.threshold_mask,
    "--scale_mask": args.scale_mask,
    "--tempmask": args.tempmask,
    "--topofile": args.topofile,
    "--aspect": args.aspect,
    "--perc_los": args.perc_los,
    "--perc_topo": args.perc_topo,
    "--crop": args.crop,
    "--crop_emp": args.crop_emp,
    "--fulloutput": args.fulloutput,
    "--geotiff": args.geotiff,
    "--plot": args.plot,
    "--dateslim": args.dateslim,
    "--nproc": args.nproc,
    "--ndatasets": args.ndatasets,
    "--cte_coh": args.cte_coh,
}
# Handle 'none' string → None for optional file args
for k in ["--aps", "--rms", "--rmspixel", "--mask", "--topofile", "--aspect",
          "--vector", "--geotiff", "--crop", "--crop_emp", "--ref_zone",
          "--steps", "--postseismic", "--slowslip", "--seasonal_increase",
          "--dateslim", "--cube"]:
    if isinstance(arguments[k], str) and arguments[k].lower() == "none":
        arguments[k] = None
...
cte_coh = float(arguments["--cte_coh"])
...
if not ds:
    logger.info(
        '.hdr file time series cube {0}, not found, open {1}'.format(arguments["--cube"], arguments["--lectfile"]))
...
if arguments["--crop"] == None:
    ...
    logger.warning(
        'Crop time series data between lines {}-{} and cols {}-{}'.format(int(crop[0]), int(crop[1]), int(crop[2]),
                                                                          int(crop[3])))
...
if arguments["--crop_emp"] == None:
    ...
    logger.warning(
        'Crop empirical estimation between lines {}-{} and cols {}-{}'.format(int(crop_emp[0]), int(crop_emp[1]),
                                                                              int(crop_emp[2]), int(crop_emp[3])))
...
if arguments["--mask"] is not None:
    ...
    if extension == ".tif":
        ...
        mask = np.fromfile(fid, dtype=np.float32).reshape(nlines, ncol)[ibeg:iend, jbeg:jend] * float(
            arguments["--scale_mask"])
        fid.close()
    maski = mask.flatten()
else:
...
if arguments["--topofile"] is not None:
    ...
    maxtopo, mintopo = np.nanpercentile(elev_map_temp, float(arguments["--perc_topo"])), np.nanpercentile(elev_map_temp,
                                                                                                          100 - float(
                                                                                                              arguments[
                                                                                                                  "--perc_topo"]))
...
if arguments["--rmspixel"] is not None:
    ...
    if extension == ".tif":
        ...
        rms_map_temp = np.fromfile(arguments["--rmspixel"], dtype=np.float32).reshape((nlines, ncol))[
            ibeg:iend, jbeg:jend]
...
def plot_displacement_maps(maps, idates, nfigure=0, cmap='RdBu', plot='yes', filename='maps.eps',
                           title='Time series maps', fig_dpi=150):
    """
...
nfigure += 1
plot_displacement_maps(maps, idates, nfigure=nfigure, cmap=cmap, plot=plot, title='Time series maps',
                       filename='maps.eps')
del maps
...
for i in range(len(pos)):
    if pos[i] > 0.:
        basis.append(
            postseismic(name='postseismic {}'.format(i), reduction='log{}'.format(i), date=cos[i], tcar=pos[i])),
...
for l in range((Mbasis)):
    basis[l].m = np.full((new_lines, new_cols), np.nan)
    basis[l].sigmam = np.full((new_lines, new_cols), np.nan)
for l in range((Mker)):
    ...
    if order == 0:
        ...
        if arguments["--topofile"] is None:
            ...
            if (ivar == 0 and nfit == 0):
                ...
                e = pars[4];
                f = pars[5]
                print('Remove ramp %f r  + %f az + %f +  %f z*az + %f z + %f z**2 for date: %i' % (a, b, c, d, e, f,
                                                                                                   idates[l]))
        ...
        if arguments["--topofile"] is None:
            ...
            if (ivar == 0 and nfit == 0):
                ...
                print('Remove ramp %f r, %f az  + %f r*az + %f + %f z + %f z**2 for date: %i' % (a, b, c, d, e, f,
                                                                                                 idates[l]))
                ...
                print('Remove ramp %f r, %f az  + %f r*az + %f + %f z + %f az*z for date: %i' % (a, b, c, d, e, f,
                                                                                                 idates[l]))
                ...
                print(
                    'Remove ramp %f r, %f az  + %f r*az + %f + + %f az*z +  %f z + %f z**2  for date: %i' % (a, b, c, d,
                                                                                                             e, f, g,
                                                                                                             idates[l]))
        ...
        if arguments["--topofile"] is None:
            ...
            if (ivar == 0 and nfit == 0):
                ...
                e = pars[4];
                f = pars[5]
                print('Remove ramp %f r**2 + %f r  + %f az + %f + %f z + %f z**2 for date: %i' % (a, b, c, d, e, f,
                                                                                                  idates[l]))
                ...
                e = pars[4];
                f = pars[5]
                print('Remove ramp %f r**2 + %f r  + %f az + %f + %f z + %f z*az for date: %i' % (a, b, c, d, e, f,
                                                                                                  idates[l]))
                ...
                f = pars[5];
                g = pars[6]
                print('Remove ramp %f r**2 + %f r  + %f az + %f + + %f z*az + %f z +%f z**2 for date: %i' % (a, b, c, d,
                                                                                                             e, f, g,
                                                                                                             idates[l]))
        ...
        if arguments["--topofile"] is None:
            ...
            if (ivar == 0 and nfit == 0):
                ...
                e = pars[4];
                f = pars[5]
                print('Remove ramp %f az**2 + %f az  + %f r + %f + %f z + %f z**2 for date: %i' % (a, b, c, d, e, f,
                                                                                                   idates[l]))
                ...
                e = pars[4];
                f = pars[5]
                print('Remove ramp %f az**2 + %f az  + %f r + %f + %f z + %f z*az for date: %i' % (a, b, c, d, e, f,
                                                                                                   idates[l]))
                ...
                h = pars[7]
                print(
                    'Remove ramp %f az**2 + %f az  + %f r + %f + %f z*az + %f z + %f z**2 + %f (z*az)**2 for date: %i' % (
                        a, b, c, d, e, f, g, h, idates[l]))
        ...
        if arguments["--topofile"] is None:
            ...
            if (ivar == 0 and nfit == 0):
                ...
                e = pars[4];
                f = pars[5]
                print('Remove ramp %f az**2 + %f az  + %f r**2 + %f r + %f + %f z for date: %i' % (a, b, c, d, e, f,
                                                                                                   idates[l]))
            ...
            if (ivar == 0 and nfit == 1):
                ...
                g = pars[6]
                print(
                    'Remove ramp %f az**2 + %f az  + %f r**2 + %f r + %f + %f z + %f z**2  for date: %i' % (a, b, c, d,
                                                                                                            e, f, g,
                                                                                                            idates[l]))
                ...
                f = pars[5];
                g = pars[6]
                print('Remove ramp %f az**2 + %f az  + %f r**2 + %f r + %f + %f z + %f az*z for date: %i' % (a, b, c, d,
                                                                                                             e, f, g,
                                                                                                             idates[l]))
                ...
                h = pars[7]
                print(
                    'Remove ramp %f az**2 + %f az  + %f r**2 + %f r + %f +  %f az*z + %f z + %f z**2 for date: %i' % (a,
                                                                                                                      b,
                                                                                                                      c,
                                                                                                                      d,
                                                                                                                      e,
        ...
        if arguments["--topofile"] is None:
            ...
            e = pars[4];
            f = pars[5]
            print('Remove ramp %f az**3 + %f az**2  + %f az + %f r**2 + %f r + %f for date: %i' % (a, b, c, d, e, f,
                                                                                                   idates[l]))
            ...
            if (ivar == 0 and nfit == 0):
                ...
                g = pars[6]
                print(
                    'Remove ramp %f az**3 + %f az**2  + %f az + %f r**2 + %f r + %f + %f z for date: %i' % (a, b, c, d,
                                                                                                            e, f, g,
                                                                                                            idates[l]))
            ...
            if (ivar == 0 and nfit == 1):
                ...
                h = pars[7]
                print(
                    'Remove ramp %f az**3 + %f az**2  + %f az + %f r**2 + %f r + %f + %f z + %f z**2 for date: %i' % (a,
                                                                                                                      b,
                                                                                                                      c,
                                                                                                                      d,
                                                                                                                      e,
                ...
                h = pars[7]
                print(
                    'Remove ramp %f az**3 + %f az**2  + %f az + %f r**2 + %f r + %f + %f z + %f z*az for date: %i' % (a,
                                                                                                                      b,
                                                                                                                      c,
                                                                                                                      d,
                                                                                                                      e,
                ...
                k = pars[9]
                print(
                    'Remove ramp %f az**3 + %f az**2  + %f az + %f r**2 + %f r + %f z*az + %f + %f z + %f z**2 + %f (z*az)**2 for date: %i' % (
                        a, b, c, d, e, f, g, h, i, k, idates[l]))

                # plot phase/elev_map
                funct = a * az ** 3 + b * az ** 2 + c * az + d * rg ** 2 + e * rg + f + g * topo_clean * az + k * (
                            topo_clean * az) ** 2
                funcbins = a * az ** 3 + b * az ** 2 + c * az + d * rg ** 2 + e * rg + f + g * topo_clean * az + k * (
                            topo_clean * az) ** 2
                x = np.linspace(mintopo, maxtopo, 100)
                # ax_dphi.scatter(topo_clean,los_clean-funct, s=0.01, alpha=0.3, rasterized=True)
                # ax_dphi.plot(topo_clean,losbins - funcbins,'-r', lw =1., label='sliding median')
        ...
        if arguments["--topofile"] is None:
            ...
            if (ivar == 0 and nfit == 0):
                ...
                print('Remove ramp %f r + %f az  + %f (r*az)**2 + %f r*az + %f + %f z for date: %i' % (a, b, c, d, e, f,
                                                                                                       idates[l]))
            ...
            if (ivar == 0 and nfit == 1):
                ...
                print(
                    'Remove ramp %f r + %f az  + %f (r*az)**2 + %f r*az + %f + %f z + %f z**2  for date: %i' % (a, b, c,
                                                                                                                d, e, f,
                                                                                                                g,
                                                                                                                idates[
                                                                                                                    l]))
                ...
                print(
                    'Remove ramp %f r + %f az  + %f (r*az)**2 + %f r*az + %f + %f z + %f az*z for date: %i' % (a, b, c,
                                                                                                               d, e, f,
                                                                                                               g,
                                                                                                               idates[
                                                                                                                   l]))
                ...
                print(
                    'Remove ramp %f r + %f az  + %f (r*az)**2 + %f r*az + %f + %f az*z + %f z + %f z**2  for date: %i' % (
                        a, b, c, d, e, f, g, h, idates[l]))
...
def empirical_cor(t, disp_map, model_map, elev_map, aspect_map, rms_map, ibeg_emp, iend_emp, mintopo, maxtopo, topofile,
                  perc_los, threshold_rms, threshold_mask, emp_sampling):
    """
    ...
    logger.debug(
        'Apply gaussian filter with an abitrary half-window size of 3 for empirical estimations to remove outliers')
    ...
    if np.nansum(disp_map) != 0:
        ...
        maxlos, minlos = np.nanpercentile(map_temp[ibeg_emp:iend_emp, jbeg_emp:jend_emp],
                                          float(perc_los)), np.nanpercentile(
            map_temp[ibeg_emp:iend_emp, jbeg_emp:jend_emp], 100 - float(perc_los))
        ...
        cols: {8} -- {9}'.format(float(threshold_rms), float(threshold_mask), minlos, maxlos, mintopo, maxtopo,
                                 ibeg_emp, iend_emp, jbeg_emp, jend_emp))
            sys.exit()
        ...
        map_ramp, map_flata, map_topo, rmsi = estim_ramp(los, los_clean[::samp], topo_clean[::samp], x[::samp], \
                                                         y[::samp], temp_flat, rms_clean[::samp], nfit_temp, ivar_temp,
                                                         l, arguments["--cond"])
        ...
        if (lin_start is not None) and (lin_end is not None):
            ...
            logger.info('Re-estimation of a constant within lines {0}-{1} and cols {2}-{3}'.format(lin_start, lin_end,
                                                                                                   col_start, col_end))
...
def empirical_cor_wrapper(l, ibeg_emp, iend_emp, mintopo, maxtopo, topofile, perc_los, threshold_rms, threshold_mask,
                          emp_sampling):
    ...
    if topofile is not None:
        ...
        _global_data['maps_flata'][:, :, l], _global_data['maps_topo'][:, :, l], _global_data['residuals_emp'][
            l] = empirical_cor(
...
def temporal_decomp_chunk(chunk, uncertainty, cond, ineq, equality):
    # chunk: (N, nb_pixels), uncertainty: per-image (N,)
    N, P = chunk.shape
    ...
    for i in range(P):
        m, sigmam, model = temporal_decomp(chunk[:, i], uncertainty, cond, ineq, equality)
        m_all[i, :] = m
        sigmam_all[i, :] = sigmam
        models_all[i, :] = model
...
def temporal_decomp(disp, uncertainty, cond, ineq, equality):
    """
    ...
    if kk > N / 6:
        ...
        sigmam = sigmam.astype(np.float32)
        mdisp[k] = (G @ m).astype(np.float32)
...
for ii in range(int(arguments["--niter"])):
    ...
    if (ii == 0) or (arguments["--spatialiter"] == 'yes'):
        ...
        with multiprocessing.Pool(processes=nprocess, initializer=init_worker,
                                  initargs=('disp_cumul_clean', 'disp_cumul_flat', 'disp_cumul_models', 'elev_map',
                                            'aspect_map', 'rms_map', (new_lines, new_cols), (new_lines, new_cols, N),
                                            'float32')) as pool:
            results = pool.starmap(empirical_cor_wrapper, [
                (l, ibeg_emp, iend_emp, mintopo, maxtopo, arguments["--topofile"], arguments["--perc_los"],
                 arguments["--threshold_rms"], arguments["--threshold_mask"], arguments["--emp_sampling"]) for l in
                range(N)])
    ...
    if N < 30:
        ...
        plot_displacement_maps(maps_flata, idates, nfigure=nfigure, cmap=cmap, plot=plot,
                               title='Corrected time series maps from empirical estimations', filename='maps_flat.eps')
        del maps_flata

        if arguments["--topofile"] is not None:
            nfigure += 1
            maps_topo = np.memmap('maps_topo', dtype='float32', mode='r', shape=(new_lines, new_cols, N))
            plot_displacement_maps(maps_topo, idates, nfigure=nfigure, cmap=cmap, plot=plot,
                                   title='Time series RAMPS+TROPO', filename='maps_models_tropo.eps')
            del maps_topo
    ...
    with TimeIt():
        with multiprocessing.Pool(processes=nproc, initializer=init) as pool:
            for line in range(0, new_lines, block_size):
                ...
                # async read of N bands then stack → (N, block_lines*ncols)
                bands_data = [r.get() for r in [
                    pool.apply_async(read_band, args=(i + 1, 'disp_cumul_flat', line, new_cols, block_size_local)) for i
                    in range(N)]]
                line_time_series = np.stack(bands_data, axis=0).reshape(N, new_cols * block_size_local)
                ...
                results_write = [
                    pool.apply_async(write_band, args=(i + 1, 'disp_cumul_models', models_temp[:, :, i], 0, line)) for i
                    in range(N)]
                [res.get() for res in results_write]  # attend la fin des écritures

                # Store coefficients — vectorised reshape instead of pixel loop
                m_block = m.reshape(block_size_local, new_cols, M)
                sigmam_block = sigmam.reshape(block_size_local, new_cols, M)
                for l in range(Mbasis):
                    basis[l].m[line:end_line, :] = m_block[:, :, l]
                    basis[l].sigmam[line:end_line, :] = sigmam_block[:, :, l]
                for l in range(Mker):
                    kernels[l].m[line:end_line, :] = m_block[:, :, Mbasis + l]
                    kernels[l].sigmam[line:end_line, :] = sigmam_block[:, :, Mbasis + l]
...
if N < 30:
    ...
    nfigure += 1
    plot_displacement_maps(models, idates, nfigure=nfigure, cmap=cmap, plot=plot, title='Time series models',
                           filename='maps-time-models.eps')
    nfigure += 1
    plot_displacement_maps(maps_flat - models, idates, nfigure=nfigure, cmap=cmap, plot=plot,
                           title='Time series residuals', filename='maps-residuals.eps')
